...
def save_predicted_mask(preds, img_name, save_to):
    preds = np.array(preds.cpu().detach().numpy())[0]

    pred_label = np.zeros((preds.shape[0], preds.shape[1], 3)).astype(np.uint8)
    pred_label[preds == 1] = [255, 0, 0]  
    pred_label[preds == 2] = [0, 255, 0]
    pred_label[preds == 3] = [0, 0, 255]

    pred_label = Image.fromarray(pred_label)
    
    img_name = img_name[0].split("/")[-1]
    pred_label.save(os.path.join(save_to, img_name).replace(".jpg", ".png"), format="PNG")
